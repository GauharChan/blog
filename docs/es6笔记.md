...
> 解构赋值时，如果等号右边是数值和布尔值，则会先转为对象。即转换为Number对象，和Boolean对象
...
当然也可以设置默认值，**但注意是给参数的值(也就是变量x,y)** 设置默认值
...
向Set加入值得时候不会进行类型转换，如：5  和  '5'  是不同的值。Set 内部判断两个值是否不同，使用的算法叫做“Same-value-zero equality”，它类似于精确相等运算符（`===`），主要的区别是向 Set 加入值时认为`NaN`等于自身，而精确相等运算符认为`NaN`不等于自身。
...
for (let item of set.values()) {// 默认生成的遍历器是values()，所以这里也可以省略
  console.log(item);
}
// red
// green
